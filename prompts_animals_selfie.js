...
const generateBatch = (count = 20) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const location = pickN(landmarks, Math.floor(1));

    lines.push(
      `videos[${i}] = \`photorealistic video of ${animals} taking a selfie - the iphone camera being used should have it's back towards the camera so the screen is not visible - the animals stand in front of ${location}\`;`
    );
  }
